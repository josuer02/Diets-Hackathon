...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://34.72.175.109";
...
export default function Home() {
  ...
  const fetchDietPlans = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_URL}/api/diet-plans`);
      if (!response.ok) {
  ...
  const handleUpdate = async (id: number) => {
    const response = await fetch(`${API_URL}/api/diet-plans/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, description, calories: parseInt(calories) }),
    });
